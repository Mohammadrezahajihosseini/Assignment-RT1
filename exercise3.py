...
while 1:
    ...
    if -a_ths<= silver_rot_y <= a_ths: # if the robot is well aligned with the silver token, we go forward
        ...
        print("Right a bit...")
        turn(+10, 0.1)         
    if -a_thg<= golden_rot_y <= a_thg: #if the robot is oriented towards the golden token, we go back
        ...
        turn (10,0.1)
        drive(5,0.1)
    elif golden_rot_y >= d_thg:#if the robot is approaching from the sides to golden token we turn left or right
